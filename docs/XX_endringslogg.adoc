...
Vi følger DCAT-AP som følger DCAT med hensyn til inverse egenskaper (se https://www.w3.org/TR/vocab-dcat-3/#inverse-properties[§ 7 i DCAT &#x29C9;, window="_blank", role="ext-link"]), at bare primæregenskapene (og ikke deres inverse egenskaper) tas med i spesifikasjonen. Alle inverse egenskaper er derfor utgått fra denne versjonen av DCAT-AP-NO. Man _kan_ bruke en invers egenskap, _i tillegg til_ og _ikke istedenfor_  primæregenskapen. 
