...
== Endringer fra v.3.0.0 til v.3.0.x

Kun redaksjonelle justeringer: 

* Fra v. 3.0.6 til v. 3.0.7: 
** Verdiområdet/_range_ for egenskapene <<Tidsrom-sluttdato, Tidsrom – sluttdato/tid (`dcat:endDate`)>> og <<Tidsrom-startdato, Tidsrom – startdato/tid (`dcat:startDate`)>> er utvidet til også å kunne være `xsd:gYear` og `xsd:gYearMonth`, i tillegg til `xsd:date` og `xsd:dateTime`. Dette for å rette opp feilen at disse to var glemt å tas med ved overgangen fra versjon 2 til versjon 3.
** Det norske navnet på egenskapen `dcat:keyword` er endret fra «emneord» til «nøkkelord» for å være mer i samsvar med det engelske navnet "keyword", og å samkjøre med andre spesifikasjoner. 

* fra v. 3.0.5 til v. 3.0.6:
** Retter opp kravnivå for de to valgfrie egenskapene i klassen Rettighetserklæring (kravnivåene rettes fra Anbefalt til Valgfri)

* fra v. 3.0.3 til v. 3.0.4:
** Rettet opp feil kardinalitet for egenskapen valuta (`cv:currency`) i <<img-Klassen-Gebyr>>.
** I endringsloggen føyet til info om at URIen for egenskapen <<Datasett-harVersjon, Datasett – har versjon (`dcat:hasVersion`)>> er endret fra `dct:hasVersion` til `dcat:hasVersion`.

* fra v. 3.0.0 til v.3.0.3: Rettet opp en del feil og mangler mtp. UU-krav (bl.a. lagt inn alternativ tekst til figurene som manglet den).  

== Endringer fra v. 2.2 til v.3.0.0 [[Endringer-fra-v2-til-v3]]

=== Endringer av dokumentstrukturen fra versjon 2.2 til v. 3.0.0 
...
I henhold til DCAT-AP som følger DCAT når det gjelder inverse egenskaper, er inverse egenskaper ikke eksplisitt tatt med i standarden. Man __kan__ bruke en invers egenskap, _i tillegg til_ og _ikke istedenfor_ egenskapen som er eksplisitt tatt med i standarden. Se <<Om-inverse-egenskaper>>.
...
*** <<Datasett-harVersjon>>: EgenskapsURIen er endret fra `dct:hasVersion` til `dcat:hasVersion`, i samsvar med DCAT-AP/DCAT. 
**** URIen for den inverse egenskapen er også endret tilsvarende, se under <<Om-inverse-egenskaper>>. 
...
*** <<Datasett-nøkkelord>>: Verdiområdet endret fra rdfs:Literal til rdf:langString som er en subklasse av rdfs:Literal. 
...
*** <<Datasett-tema>>: EuroVoc er ikke lenger med i kravet til bruk av kontrollerte vokabularer, i samsvar med DCAT-AP/DCAT.
...
*** <<Datatjeneste-har-gebyr>>, for å kunne oppgi ev. kostnader knyttet til bruk av datatjenesten.
...
*** <<Datatjeneste-nøkkelord>>: Verdiområdet endret fra rdfs:Literal til rdf:langString som er en subklasse av rdfs:Literal. 
...
** *Endrede krav til bruk av kontrollerte vokabularer*: 
*** <<Datatjeneste-tema>>: "EuroVoc" er ikke lenger med i kravet til bruk av kontrollerte vokabularer, i samsvar med DCAT-AP/DCAT. 
...
** *Endrede krav til bruk av kontrollerte vokabularer*: 
*** <<Katalog-produsent>>: Kravet om bruk av EUs kontrollerte vokabular "Corporate body" tatt bort, i samsvar med DCAT-AP/DCAT.
*** <<Katalog-temaer>>: "EuroVoc" er ikke lenger med, og "Data theme" tilføyet, i kravet om bruk av kontrollerte vokabularer, i samsvar med DCAT-AP/DCAT. 
