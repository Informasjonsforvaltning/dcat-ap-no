...
| Merknad 2 / _Note 2_ | Norsk utvidelse: Når verdien er begrenset tilgang (`RESTRICTED`) eller ikke-allmenn tilgang (`NON_PUBLIC`), BØR egenskapen <<Datasett-gjeldendeLovgivning>> brukes. 

__When then value is `RESTRICTED` or `NON_PUBLIC`, the property <<Datasett-gjeldendeLovgivning>> SHOULD be used.__
