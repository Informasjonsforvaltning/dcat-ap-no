...
<<diagram-Klassen-Datasett>> viser klassen Datasett og dens egenskaper, samt klassene den refererer til. 
...
| Multiplisitet / _Multiplicity_ | 0..n
| Kravnivå / _Requirement level_ | Anbefalt / _Recommended_
| Eksempel | http://vegkart.no[http://vegkart.no &#x29C9;, window="_blank", role="ext-link"] er en distribusjon av datasettet Nasjonal vegdatabank (NVDB), i html (menneskelesbar visning).
|===

Eksempel i RDF Turtle:
-----
<datasettNVDB> a dcat:Dataset ; 
   dcat:distribution <htmlNVDB> ; 
   .

<htmlNVDB> a dcat:Distribution ; 
   dcat:accessURL <http://vegkart.no> ; 
...
__Norwegian extension: To specify spatial coverage in Norway, the Norwegian Mapping Authority’s controlled vocabulary https://data.geonorge.no/administrativeEnheter/nasjon/doc/173163[Administrative units &#x29C9;, window="_blank", role="ext-link"] SHOULD be used in addition.__
...
| Eksempel / _Example_ | https://lovdata.no/dokument/SF/forskrift/2016-12-15-1600[Forskrift om tilgjengeliggjøring av sanntids veg- og trafikkinformasjonstjenester &#x29C9;, window="_blank", role="ext-link"]
|===
...
=== Datasett – nøkkelord (dcat:keyword) [[Datasett-nøkkelord]]
...
| URI | dcat:keyword
| Verdiområde / _Range_ | rdf:langString
| Anvendelse / _Usage note_ | Egenskapen brukes til å oppgi nøkkelord som beskriver datasettet. 
...
| Merknad 2 / _Note 2_ | Norsk utvidelse: Når verdien er "begrenset tilgang (`RESTRICTED`)" eller "ikke-allmenn tilgang (`NON_PUBLIC`)", BØR egenskapen <<Datasett-gjeldendeLovgivning>> brukes. I tilfeller der "begrenset tilgang (`RESTRICTED`)" ikke er alltid regulert av lovgivning, BØR egenskapen <<Distribusjon-policy>> brukes.  

__Norwegian extension: When the value is `RESTRICTED` or `NON_PUBLIC`, the property <<Datasett-gjeldendeLovgivning>> SHOULD be used. In cases where `RESTRICTED` is not regulated by any legislation, the property <<Distribusjon-policy>> SHOULD be used.__
| Merknad 3 / _Note 3_ | Norsk utvidelse: Kravnivået er endret fra valgfri til anbefalt.
...
=== Datasett – annen ansvarlig aktør (prov:qualifiedAttribution) [[Datasett-annenAnsvarligAktør]]
...
| Anvendelse / _Usage note_ | Egenskapen brukes til å referere til en aktør som har en eller annen form for ansvar for datasettet, men som ikke er en <<Datasett-utgiver, utgiver>> eller <<Datasett-produsent, produsent>> av datasettet.

__This property is used to refer to an agent that has some form of responsibility for the dataset, but which is not a <<Datasett-utgiver, publisher>> nor a <<Datasett-produsent, creator>> of the datasett.__
| Multiplisitet / _Multiplicity_ | 0..n
| Kravnivå / _Requirement level_ | Valgfri / _Optional_ 
| Merknad / _Note_ | Denne egenskapen er ment å brukes til å referere til en aktør som har en gitt rolle, annen enn utgiver, produsent osv., f.eks. distributør. Se https://www.w3.org/TR/vocab-dcat-3/#qualified-attribution[forklaring og eksempler i W3C/DCAT &#x29C9;, window="_blank", role="ext-link"].

__This property is meant to be used to refer to an agent that has a given role, other than publisher, creator etc., e.g., a distributor. See https://www.w3.org/TR/vocab-dcat-3/#qualified-attribution[explanations and examples in W3C/DCAT &#x29C9;, window="_blank", role="ext-link"].__
|===
...
=== Datasett – annen spesifikk relasjon (dcat:qualifiedRelation) [[Datasett-annenSpesifikkRelasjon]]
...
| Anvendelse / _Usage note_ | Egenskapen brukes til å referere til en beskrivelse av en spesifikk relasjon som datasettet har til en annen ressurs, og som ikke kan uttrykkes ved bruk av egenskapene <<Datasett-erstatter, erstatter>>, <<Datasett-kilde, kilde>>, <<Datasett-erReferertAv, er referert av>> osv. 

_This property is used to refer to a description of a relationship that the dataset has with another resource, and which can not be expressed using properties <<Datasett-erstatter, replaces>>, <<Datasett-kilde, source>>, <<Datasett-erReferertAv, is referenced by>> etc._
| Multiplisitet / _Multiplicity_ | 0..n
| Kravnivå / _Requirement level_ | Valgfri / _Optional_
| Merknad / _Note_ | Egenskapen er ment for å brukes til å uttrykke relasjoner som ikke kan uttrykkes ved bruk av egenskapene <<Datasett-erstatter>>, <<Datasett-erReferertAv>>, <<Datasett-forrige>>, <<Datasett-harVersjon>>, <<Datasett-kilde>> osv., og når man ønsker å unngå å bruke den uspesifikke relasjonen <<Datasett-relatertRessurs>>. Se https://www.w3.org/TR/vocab-dcat-3/#qualified-relationship[forklaring og eksempler i W3C/DCAT &#x29C9;, window="_blank", role="ext-link"].

__This property is meant to be used to express relationships that can not be expressed using the properties <<Datasett-erstatter>>, <<Datasett-erReferertAv>>, <<Datasett-forrige>>, <<Datasett-harVersjon>>, <<Datasett-kilde>> etc., and when avoiding using the unspecific relation <<Datasett-relatertRessurs>>. See https://www.w3.org/TR/vocab-dcat-3/#qualified-relationship[explanations and examples in W3C/DCAT &#x29C9;, window="_blank", role="ext-link"].__
|===
...
=== Datasett – eierskapshistorikk (dct:provenance) [[Datasett-eierskapshistorikk]]
...
| Anvendelse / _Usage note_ | Egenskapen brukes til å referere til beskrivelse av endring i eierskap og forvaltning av datasett (fra det ble skapt) som har betydning for autentisitet, integritet og fortolkning.
...
| URI | dcat:prev
| Verdiområde / _Range_ | dcat:Dataset
| Anvendelse / _Usage note_ | Egenskapen brukes til å referere til det forrige datasettet i en ordnet samling eller serie av datasett.
...
| Eksempel | For å uttrykke at et datasett er autoritativt, kan man bruke denne egenskapen til å referere til `dqvno:isAuthoritative` som er en predefinert instans av klassen Kvalitetssertifikat (`dqv:QualityCertificate`). 
|===
...
=== Datasett – har kvalitetsmåling (dqv:hasQualityMeasurement) [[Datasett-harKvalitetsmåling]]
...
| Anvendelse / _Usage note_ | Egenskapen brukes til å referere til en kvalitetsmåling. 

_This property is used to refer to a quality measurement._
| Multiplisitet / _Multiplicity_ | 0..n
| Kravnivå / _Requirement level_ | Valgfri / _Optional_
| Merknad / _Note_ | Se https://data.norge.no/specification/dqv-ap-no[DQV-AP-NO (norsk applikasjonsprofil av DQV) &#x29C9;, window="_blank", role="ext-link"] for bruk av klassen `dqv:QualityMeasurement`.

__See the Norwegian application profile of DQV, https://data.norge.no/specification/dqv-ap-no[DQV-AP-NO &#x29C9;, window="_blank", role="ext-link"], for the usage of the class `dqv:QualityMeasurement`.__ 
|===
...
| URI | dct:relation
| Verdiområde / _Range_ | rdfs:Resource
| Anvendelse / _Usage note_ | Egenskapen brukes til å referere til en beslektet ressurs, uten nærmere angivelse av type relasjon.

_This property is used to refer to a related resource, without specifying further type of the relationship._
| Multiplisitet / _Multiplicity_ | 0..n
| Kravnivå / _Requirement level_ | Valgfri / _Optional_
| Merknad / _Note_ | Denne egenskapen brukes når det ikke er behov for nærmere angivelse av type relasjon datasettet har med den andre ressursen. Bruk heller egenskapene <<Datasett-erstatter>>, <<Datasett-erReferertAv>>, <<Datasett-forrige>>, <<Datasett-harVersjon>>, <<Datasett-kilde>> osv., eller <<Datasett-annenSpesifikkRelasjon>>, når det er behov for å angi type relasjon. 

__This property is used when there is no need for further specifying type of the relationship that the dataset has with the other resource. Use rather properties <<Datasett-erstatter>>, <<Datasett-erReferertAv>>, <<Datasett-forrige>>, <<Datasett-harVersjon>>, <<Datasett-kilde>> etc., or <<Datasett-annenSpesifikkRelasjon>>, when the type of the relationship needs to be specified.__
|===
