...
| Datasett – nøkkelord | dcat:keyword |
Verdiområdet endret fra rdfs:Literal til rdf:langString som er en subklasse av rdf:Literal. 
...
__To specify spatial coverage in Norway, the Norwegian Mapping Authority’s controlled vocabulary https://data.geonorge.no/administrativeEnheter/nasjon/doc/173163[Administrative units &#x29C9;, window="_blank", role="ext-link"] SHOULD be used in addition.__
...
__The requirement level is changed from optional to recommended.__
| Datasett – tilgangsrettigheter | dct:accessRights | Når verdien er "begrenset tilgang (`RESTRICTED`)" eller "ikke-allmenn tilgang (`NON_PUBLIC`)", BØR egenskapen <<Datasett-gjeldendeLovgivning>> brukes. I tilfeller der "begrenset tilgang (`RESTRICTED`)" ikke er alltid regulert av lovgivning, BØR egenskapen <<Distribusjon-policy>> brukes.

__When the value is `RESTRICTED` or `NON_PUBLIC`, the property <<Datasett-gjeldendeLovgivning>> SHOULD be used. In cases where `RESTRICTED` is not regulated by any legislation, the property <<Distribusjon-policy>> SHOULD be used.__
...
| Datatjeneste – nøkkelord | dcat:keyword |
Verdiområdet endret fra rdfs:Literal til rdf:langString som er en subklasse av rdf:Literal. 
